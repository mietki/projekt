...
<div class="divA">
<div class="title">
<font size="+2"><P ALIGN="CENTER">EDYCJA KONTAKTU</P></font>
</div>
<div class="divB">
<div class="divD">
<p>Wybierz datę aktualizacji kontaktu, który chcesz edytować</p>
<?php
...
$query = mysql_query("update kontakt set
data='$data', 
opis='$opis'

where id_kon='$id'", $connection);
}
$query = mysql_query("select * from kontakt", $connection);
while ($row = mysql_fetch_array($query)) {
echo "<b><a href='edit_kontakt.php?update={$row['id_kon']}'>{$row['data']}</a></b>";
echo "<br />";
}
?>
</div><?php
...
$update = $_GET['update'];
$query1 = mysql_query("select * from kontakt where id_kon=$update", $connection);
while ($row1 = mysql_fetch_array($query1)) {
echo "<form class='form' method='get'>";

echo "<hr/>";
echo"<input class='input' type='hidden' name='did_kon' value='{$row1['id_kon']}' />";

?>
<p align="left"><b>DATA OSTATNIEJ AKTUALIZACJI:</B> </br>
...
<Span>Kontakt został zaktualizowany</span></div>';
}
?>
<div class="clear"></div>
</div>
